...
$(function () {
    ...
    //发起付款
    function pay(final_value) {
        cover.show();
        //获取请求的url
        var url_value = $('.urlvalue').val();
        //var url_value="url.json";
        $.ajax({
            url: url_value,
            data: {amount: final_value, remark: remark.val()},
            dataType: "json",
            type: 'post',
            success: function (res_data) {
                if (res_data.result.success) {
                    if (res_data.data.pay_type == "wechat") {
                        weixin_pay_action(res_data.data);
                    }
                    else {
                        window.location.href = res_data.data.redirect_url;
                    }
                }
                else {
                    alert(res_data.result.errorMsg);
                }
            },
            error: function () {
                alert('服务器请求失败')
